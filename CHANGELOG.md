...
### Added
- `--version` and `-V` flags for displaying the installed `rfc-cli` version.

## [0.1.6]

### Added
- Read-only MCP server via `rfc-cli mcp` over stdio with RFC tools for listing,
  viewing, checking status, and inspecting dependencies.
- Automatic `rfc-cli` MCP server configuration in `.mcp.json`, preserving other
  configured servers.
