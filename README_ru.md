...
| `RFC_VIEWER` | Внешний рендерер Markdown для команды `view` (получает содержимое через stdin) | — (сырой вывод в stdout) |
...
Если задана переменная окружения `RFC_VIEWER`, содержимое передаётся этой программе (например, терминальному рендереру Markdown) вместо сырого вывода:

```sh
export RFC_VIEWER="mdt"
rfc-cli view 1          # рендерится через mdt

rfc-cli view 1 --raw    # принудительно сырой Markdown, игнорируя RFC_VIEWER
```
